...
# dataset for performing augmentations (this was added after training the MTL model to allow
# for a fair comparison between both)
class FeatureMapDatasetImproved(Dataset):
      def __init__(self, base_dir, mode, rescale=True, augment=False):
            self.base_dir = os.path.abspath(base_dir)
            self.split = mode
            self.rescale = rescale
